...
A shared parity harness runs each port side by side with the original and
compares output and resulting files; both pass all 89 cases. They also install
large skills much faster, because each skill is copied once per run instead of
once per agent.
...
Apart from saying `skills …` instead of `npx skills …` in help and hints, the
ports aim to behave identically. Deliberate and known differences are listed
in [`rust/PARITY.md`](rust/PARITY.md) and [`dotnet/PARITY.md`](dotnet/PARITY.md).
